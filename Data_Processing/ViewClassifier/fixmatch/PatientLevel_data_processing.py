...
def main(split, result_save_dir, class_to_integer_mapping_dir, suggested_split_file_path_labeledpart, suggested_split_file_path_unlabeledpart, raw_data_rootdir):
    ...
    with open(os.path.join(result_save_dir, 'PatientLevel_{}_image.npy'.format(split)), 'wb') as f:
        np.save(f, image_list)
    
    with open(os.path.join(result_save_dir, 'PatientLevel_{}_label.npy'.format(split)), 'wb') as f:
        np.save(f, viewlabel_list)
