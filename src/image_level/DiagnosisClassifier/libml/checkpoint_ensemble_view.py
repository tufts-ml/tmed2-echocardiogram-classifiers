...
import os
import sys
from libml.utils import retrieve_TestAccuracy_at_MaxValidationAccuracy, init_composition, ensemble, test_prediction_forward_stepwise, save_pickle, load_pickle



def perform_ensemble_view(experiment_dir, result_save_dir, num_selection_step, report_type, ensemble_last_checkpoints=100, task_name='ViewClassification'):
    
    #define predictions_dir
    predictions_dir = os.path.join(experiment_dir, task_name+'_predictions')
    stanford_test_predictions_dir = os.path.join(experiment_dir, 'predictions')
    
    result_save_dir = result_save_dir + '_{}'.format(task_name)
    ...
    assert len(valid_predictions_file_list) == len(test_predictions_file_list) == len(stanford_test_predictions_file_list), 'number test pkl not equal to number valid pkl'
...
#     bagging_test_accuracy, _ = simple_bagging(test_predictions_used_for_ensemble, predictions_dir, report_type)
    ...
    valid_composition, valid_best_ensemble_accuracy, _ = ensemble(predictions_dir, result_save_dir, num_selection_step, valid_predictions_used_for_ensemble, initial_composition, report_type, task_name)
    
    test_ensemble_accuracy, test_ensemble_predictions, test_composition = test_prediction_forward_stepwise(predictions_dir, valid_composition['best_composition'], report_type, task_name)
    ...
    if report_type == 'RAW_BalancedAccuracy':
        output_string1 = 'NO ensemble: max RAW validation balanced accuracy is {}, RAW test balanced accuracy at max RAW validation balanced accuracy is {}, RAW stanford test accuracy is {}\n'
#         output_string2 = 'Bagging, last {} checkpoints: RAW valid balanced accuracy is {}, RAW test balanced accuracy is {}'
        ...
        output_string1 = 'NO ensemble: max EMA validation balanced accuracy is {}, EMA test balanced accuracy at max EMA validation balanced accuracy is {}, EMA stanford test accuracy is {}\n'
#         output_string2 = 'Bagging, last {} checkpoints: EMA valid balanced accuracy is {}, EMA test balanced accuracy is {}'
    ...
    file_writer.write(output_string1.format(max_valid_accuracy, test_accuracy_at_max_valid_epoch, stanford_test_accuracy_at_max_val_accuracy))
