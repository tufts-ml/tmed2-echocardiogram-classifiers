...
def generate_bootstrap_accuracy_curve(original_predictions, original_labels, num_bootstrap_samples):
    ...
    for j in range(len(original_predictions)): #len(original_predictions) is how many steps
        ...
        balanced_accuracy = calculate_balanced_accuracy(original_labels[j], original_predictions[j].argmax(1),'diagnosis')
...
#         print('i is {}, bootstrap_ix is {}'.format(i, bootstrap_ix))
        ...
        for j in range(len(bootstrap_predictions)): #each element is the predictions at one step
            ...
            balanced_accuracy = calculate_balanced_accuracy(bootstrap_true_labels[j], bootstrap_predictions[j].argmax(1), 'DiagnosisClassification')
...
def perform_analysis_diagnosis(figure_title, experiment_dir, result_save_dir, num_bootstrap_samples, bootstrap_upper_percentile, bootstrap_lower_percentile, ylim_lower, ylim_upper, report_type, task_name):
    ...
    predictions_dir = os.path.join(experiment_dir, 'DiagnosisClassification_predictions')
