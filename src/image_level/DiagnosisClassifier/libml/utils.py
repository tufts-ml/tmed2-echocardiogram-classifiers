...
#same used in Echo_ClinicalManualScript_1112/ViewClassifier
def customized_CE(combined_labels, combined_logits, UsefulUnlabeled_batch_size):
    ...
    loss_normal_CE = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels_for_normal_CE, logits=logits_for_normal_CE)
    ...
    labels_for_UsefulUnlabeled = combined_labels[-UsefulUnlabeled_batch_size:, :]
    
    normalized_labels_for_UsefulUnlabeled = tf.concat([tf.gather(labels_for_UsefulUnlabeled, indices=[0,1], axis=1), tf.reduce_sum(tf.gather(labels_for_UsefulUnlabeled, indices=[2,3,4], axis=1), axis=1, keepdims=True)], axis=1)
    
    softmax_logits_for_UsefulUnlabeled = tf.exp(logits_for_UsefulUnlabeled)/tf.reduce_sum(tf.exp(logits_for_UsefulUnlabeled), axis=1, keepdims=True)
    normalized_softmax_logits_for_UsefulUnlabeled = tf.concat([tf.gather(softmax_logits_for_UsefulUnlabeled, indices=[0,1], axis=1), tf.reduce_sum(tf.gather(softmax_logits_for_UsefulUnlabeled, indices=[2,3,4], axis=1), axis=1, keepdims=True)], axis=1)
    
    loss_UsefulUnlabeled = -tf.reduce_sum(normalized_labels_for_UsefulUnlabeled * tf.log(tf.clip_by_value(normalized_softmax_logits_for_UsefulUnlabeled, 1e-10, 1.0)) , axis=1)
    print('loss_UsefulUnlabeled: {}'.format(loss_UsefulUnlabeled), flush=True)
    
    return tf.concat([loss_normal_CE, loss_UsefulUnlabeled], axis=0)


#an implementation same as customized_CE above (has been tested in Echo_ClinicalManualScript_20211029)
def customized_CE_using_LabelMask_withoutlabelsmoothing(combined_labels, combined_logits):
    
    print('type of combined_labels: {}'.format(type(combined_labels), flush=True))
    print('type of combined_logits: {}'.format(type(combined_logits), flush=True))
    
    #method for creating mask: ref: https://github.com/brain-research/realistic-ssl-evaluation/blob/81fe4b2c74282fa94219ddf6a22146a6f6b2cfab/lib/ssl_framework.py#L188
    
    UsefulUnlabeled_mask = tf.equal(4, combined_labels)
    Not_UsefulUnlabeled_mask = tf.not_equal(4, combined_labels)
    logits_for_normal_CE = combined_logits[Not_UsefulUnlabeled_mask]
    labels_for_normal_CE = combined_labels[Not_UsefulUnlabeled_mask]
    labels_for_normal_CE = tf.one_hot(labels_for_normal_CE, 5)
...
#     logits_for_UsefulUnlabeled = combined_logits[-UsefulUnlabeled_batch_size:, :]
#     labels_for_UsefulUnlabeled = combined_labels[-UsefulUnlabeled_batch_size:, :]
    logits_for_UsefulUnlabeled = combined_logits[UsefulUnlabeled_mask]
    labels_for_UsefulUnlabeled = combined_labels[UsefulUnlabeled_mask]
    labels_for_UsefulUnlabeled = tf.one_hot(labels_for_UsefulUnlabeled, 5)
    
    normalized_labels_for_UsefulUnlabeled = tf.concat([tf.gather(labels_for_UsefulUnlabeled, indices=[0,1], axis=1), tf.reduce_sum(tf.gather(labels_for_UsefulUnlabeled, indices=[2,3,4], axis=1), axis=1, keepdims=True)], axis=1)
    
    normalized_logits_for_UsefulUnlabeled = tf.concat([tf.gather(logits_for_UsefulUnlabeled, indices=[0,1], axis=1), tf.reduce_sum(tf.gather(logits_for_UsefulUnlabeled, indices=[2,3,4], axis=1), axis=1, keepdims=True)], axis=1)
    
    loss_UsefulUnlabeled = tf.nn.softmax_cross_entropy_with_logits_v2(labels=normalized_labels_for_UsefulUnlabeled, logits=normalized_logits_for_UsefulUnlabeled)
    
    return tf.concat([loss_normal_CE, loss_UsefulUnlabeled], axis=0)



def customized_diagnosis_loss(combined_diagnosis_labels, combined_diagnosis_logits, weights_diagnosis):
    unweighted_diagnosis_loss = tf.nn.softmax_cross_entropy_with_logits_v2(labels=combined_diagnosis_labels, logits=combined_diagnosis_logits)
...
def customized_view_loss(combined_view_labels, combined_view_logits, weights_view):
    unweighted_view_loss = customized_CE_using_LabelMask(combined_view_labels, combined_view_logits)
    weighted_view_loss = unweighted_view_loss * weights_view
    weighted_view_loss = tf.reduce_mean(weighted_view_loss)
    
    return weighted_view_loss
...
def calculate_balanced_accuracy(true_labels, predictions, task_name, return_type = 'balanced_accuracy'):
    '''
    used particularly for this n-classes classification task
    '''
    
    confusion_matrix = sklearn_cm(true_labels, predictions)
    n_class = confusion_matrix.shape[0]
    print('Inside calculate_balanced_accuracy_view, {} classes passed in'.format(n_class), flush=True)
#     assert n_class==8
    
    recalls = []
    if task_name == 'ViewClassification':
        for i in range(n_class-1): #only calculate the balanced accuracy for the first 4 classes
            recall = confusion_matrix[i,i]/np.sum(confusion_matrix[i])
            recalls.append(recall)
            print('class{} recall: {}'.format(i, recall), flush=True)
    elif task_name == 'DiagnosisClassification':
        for i in range(n_class): #only calculate the balanced accuracy for the first 4 classes
            recall = confusion_matrix[i,i]/np.sum(confusion_matrix[i])
            recalls.append(recall)
            print('class{} recall: {}'.format(i, recall), flush=True)
...
def retrieve_TestAccuracy_at_MaxValidationAccuracy(predictions_dir, valid_predictions_file_list, test_predictions_file_list, report_type):
    ...
    if report_type == 'RAW_BalancedAccuracy':
        ...
        raise NameError('Unsupported report type (retrieve_Testx_at_MaxValidationAccuracy)')
...
def forward_stepwise_selection(model_library, predictions_dir, running_ensemble_prediction, running_ensemble_size, report_type, task_name):
    '''
...
#         accuracy_calculation_method = calculate_balanced_accuracy
    ...
    for model_name in model_library:
        ...
        ifadded_performance = calculate_balanced_accuracy(current_true_labels, ensembled_prediction.argmax(1), task_name)
...
def ensemble(predictions_dir, result_save_dir, num_steps, model_library, initial_composition, report_type, task_name):
    ...
    for i in range(1, num_steps + 1):
        ...
        model_to_add, updated_running_ensemble_prediction, ensemble_accuracy = forward_stepwise_selection(model_library, predictions_dir, current_ensemble_prediction, current_ensemble_size, report_type, task_name)
...
def test_prediction_forward_stepwise(predictions_dir, best_composition_from_validation, report_type, task_name):
    '''
...
#         accuracy_calculation_method = calculate_balanced_accuracy
    ...
    test_ensemble_accuracy = calculate_balanced_accuracy(true_labels, ensemble_predictions.argmax(1), task_name)
