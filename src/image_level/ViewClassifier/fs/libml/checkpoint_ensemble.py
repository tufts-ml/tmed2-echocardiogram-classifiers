...
def perform_ensemble(experiment_dir, result_save_dir, num_selection_step, report_type, ensemble_last_checkpoints=100):
    ...
    stanford_test_predictions_file_list = sorted([file for file in os.listdir(predictions_dir) if file.startswith('stanford_test')], key = lambda s: int(s.split('_')[3]))
    ...
    stanford_test_predictions_at_max_val_accuracy = load_pickle(predictions_dir, stanford_test_predictions_file_list[max_valid_accuracy_epoch])
